...
const DATA_FILE = path.resolve(__dirname, '../../data/lotofacil-resultados.json');

const tamanho = parseInt(process.argv[2], 10);
if (!tamanho || tamanho < 1 || tamanho > 15) {
  console.error('Uso: node sequenciasRepetidas.js <tamanho>');
  console.error('Exemplo: node sequenciasRepetidas.js 7');
  console.error('Tamanho deve ser um número entre 1 e 15.');
  process.exit(1);
}

const OUTPUT_FILE = path.resolve(__dirname, `../../data/sequencias-repetidas-${tamanho}.txt`);

/**
 * Gera todas as combinações de `k` elementos a partir de um array.
 */
...
function main() {
  ...
  // chave: "01-02-03-04-05-06-07" → lista de concursos onde apareceu
  const mapa = new Map();
  ...
  // Filtra apenas combinações que apareceram mais de uma vez
  const repetidas = [];
  ...
  // Ordena pela maior quantidade de repetições
  ...
  const linhas = [
    `Combinações de ${tamanho} números que se repetiram em concursos da Lotofácil`,
